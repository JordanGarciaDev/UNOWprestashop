...
class ProductCore extends ObjectModel
{
    ...
    /** @var string|a Short description or array of short description by id_lang */
    public $description_short;

    /** @var string|array el nuevo campo personalizable or array of by id_lang */
    public $campo_personalizable;
    ...
    public static $definition = [
        ...
        'fields' => [
            ...
            'campo_personalizable' => ['type' => self::TYPE_HTML, 'lang' => true, 'validate' => 'isCleanHtml'],
    ...
    public function validateField2($field, $value, $id_lang = null, $skip = [], $human_errors = false)
    {
        if ($field == 'campo_personalizable') {
            // The legacy validation is basic, so the idea here is to adapt the allowed limit so that it takes into
            // account the difference between the raw text and the html text (since actually the limit is only about
            // the raw text) This is a bit ugly the real validation should only be performed by TinyMceMaxLengthValidator
            // but we have to deal with this for now.
            ...
            $size_with_html = Tools::strlen($value);
            $adaptedLimit = $limit + $size_with_html - $size_without_html;
            $this->def['fields']['campo_personalizable']['size'] = $adaptedLimit;
        }

        return parent::validateField2($field, $value, $id_lang, $skip, $human_errors);
    }
    ...
    public static function getNewProducts($id_lang, $page_number = 0, $nb_products = 10, $count = false, $order_by = null, $order_way = null, Context $context = null)
    {
        ...
        $sql = new DbQuery();
        $sql->select(
            'p.*, product_shop.*, stock.out_of_stock, IFNULL(stock.quantity, 0) as quantity, pl.`description`, pl.`description_short`, pl.`campo_personalizable`, pl.`link_rewrite`, pl.`meta_description`,
    ...
    public static function getRandomSpecial($id_lang, $beginning = false, $ending = false, Context $context = null)
    {
        ...
        if ($product_reductions) {
            ...
            $sql = 'SELECT p.*, product_shop.*, stock.`out_of_stock` out_of_stock, pl.`description`, pl.`description_short`,pl.campo_personalizable,
    ...
    public static function getPricesDrop(
        ...
        $sql = '
        SELECT
            p.*, product_shop.*, stock.out_of_stock, IFNULL(stock.quantity, 0) as quantity, pl.`description`, pl.`description_short`, p.campo_personalizable,pl.`available_now`, pl.`available_later`,
    ...
    {
        $sql = 'SELECT p.*, product_shop.*, stock.out_of_stock, IFNULL(stock.quantity, 0) as quantity, pl.`description`, pl.`description_short`,pl.campo_personalizable, pl.`link_rewrite`,
